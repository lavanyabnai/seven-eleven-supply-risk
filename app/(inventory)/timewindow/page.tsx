...
export default function TimewindowsPage() {
  ...
  const onSubmitImport = async (values: typeof timewindowSchema.$inferInsert[]) => {
    ...
    const data = values.map((value) => ({
      ...value,
      daysOfWeek: Number(value.daysOfWeek)
    })) as { facilityId: number; operation: string; daysOfWeek: number; startTime: Date; endTime: Date; timePeriodId: number; }[];
      ...
      <div className="px-4">
        <DataTable
          ...
          data={timewindows.map(tw => ({
            ...tw,
            startTime: tw.startTime ? new Date(tw.startTime) : null,
            endTime: tw.endTime ? new Date(tw.endTime) : null
          }))}
          onDelete={(row) => {
